...
use BlackCat\Database\Packages\Coupons\Repository\CouponRepositoryInterface;
...
class CouponRepository implements CouponRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'code_ci' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Coupons\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'type', 'value', 'currency', 'starts_at', 'ends_at', 'max_redemptions', 'min_order_amount', 'applies_to', 'is_active', 'updated_at' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndCodeCi(int $tenantId, string $codeCi, bool $asDto = false): array|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'code_ci' => $codeCi ], $asDto);
    }
    ...
    public function getIdByTenantIdAndCodeCi(int $tenantId, string $codeCi) {
        $row = $this->getByTenantIdAndCodeCi($tenantId, $codeCi, false);
