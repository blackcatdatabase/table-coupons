...
namespace BlackCat\Database\Packages\Coupons\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Coupons\Definitions;
use BlackCat\Database\Packages\Coupons\Criteria;
use BlackCat\Database\Packages\Coupons\Dto\CouponDto as Dto;
use BlackCat\Database\Packages\Coupons\Mapper\CouponDtoMapper as RowMapper;
...
final class CouponRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Coupons\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'type', 'value', 'currency', 'starts_at', 'ends_at', 'max_redemptions', 'min_order_amount', 'applies_to', 'is_active', 'updated_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'code_ci' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Coupons\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'code_ci' ], [ 'type', 'value', 'currency', 'starts_at', 'ends_at', 'max_redemptions', 'min_order_amount', 'applies_to', 'is_active', 'updated_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null */
    public function getByTenantIdAndCodeCi(int $tenantId, string $codeCi, bool $asDto = false): array|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'code_ci' => $codeCi ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Coupons\Mapper\CouponDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndCodeCi(int $tenantId, string $codeCi): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'code_ci') . ' = :uniq_code_ci';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_code_ci' => $codeCi ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndCodeCi(int $tenantId, string $codeCi) {
        $row = $this->getByTenantIdAndCodeCi($tenantId, $codeCi);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Coupons\Dto\CouponDto|null {
