...
namespace BlackCat\Database\Packages\Coupons\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Coupons\Dto\CouponDto;
use BlackCat\Database\Packages\Coupons\Mapper\CouponDtoMapper;
use BlackCat\Database\Packages\Coupons\Repository\CouponRepository;
 ...
 */
final class CouponsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CouponRepository $couponRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'coupons' => $this->couponRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Coupons\ModuleException("Record {$idStr} not found for locking.");
            }
